...
async function applyThrottle(scope, identifier, limit, windowMs) {
  const now = new Date()
...
async function createAuthToken({ email, userId = null, type, token, expiresAt }) {
...
async function consumeAuthToken({ email, type, token }) {
...
export default class AuthService {
  ...
  static async login(email, password, deviceInfo) {
    ...
    const throttle = await applyThrottle('admin_login_email', email.toLowerCase(), 5, 15 * 60 * 1000)
    if (!throttle.allowed) {
  ...
  static async verifyOTPAndLogin(email, otp, deviceInfo) {
  ...
  static async resendOTP(email, deviceInfo) {
  ...
  static async forgotPassword(email, requestMeta = {}) {
    console.log('[ADMIN FORGOT_PASSWORD] request received', {
      email,
      requestMeta,
    ...
    try {
      console.log('[ADMIN FORGOT_PASSWORD] step: applyThrottle: start')
      ...
      console.log('[ADMIN FORGOT_PASSWORD] step: applyThrottle: result', throttle)

      if (!throttle.allowed) {
        console.log('[ADMIN FORGOT_PASSWORD] throttle blocked -> return generic success')
        return { success: true, message: 'Password reset link sent to your email' }
      }

      console.log('[ADMIN FORGOT_PASSWORD] step: user lookup start')
      const user = await prisma.user.findUnique({
        where: { email },
        include: { role: true },
      })
      console.log('[ADMIN FORGOT_PASSWORD] step: user lookup result', user ? { id: user.id, email: user.email } : null)

      if (!user) {
        console.log('[ADMIN FORGOT_PASSWORD] no user -> return generic success')
        return { success: true, message: 'Password reset link sent to your email' }
      }

      console.log('[ADMIN FORGOT_PASSWORD] step: token generation start')
      const token = crypto.randomUUID()
      console.log('[ADMIN FORGOT_PASSWORD] token generated', token)

      console.log('[ADMIN FORGOT_PASSWORD] step: authToken create start')
      await createAuthToken({
        email: user.email,
        userId: user.id,
        type: RESET_TYPE,
      ...
      console.log('[ADMIN FORGOT_PASSWORD] step: authToken create done')
      ...
      console.log('[ADMIN FORGOT_PASSWORD] step: email generation done (resetUrl computed)', {
        base: process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000',
        emailParam: encodeURIComponent(email),
        hasToken: !!token,
      })

      console.log('[ADMIN FORGOT_PASSWORD] step: sendPasswordResetEmail start')
      await sendPasswordResetEmail(
        email,
        `${user.firstName || ''} ${user.lastName || ''}`.trim() || 'Admin',
        resetUrl
      )
      console.log('[ADMIN FORGOT_PASSWORD] step: sendPasswordResetEmail done')
