...
export class LeadActivityService {
  async getAllActivities(leadId: any, limit = 50, offset = 0) {
  ...
  async logActivity(leadId: any, activityType: any, description: any, createdBy: any) {
    const pool = await getMysqlPool()
  ...
  async logStatusChange(leadId: any, oldStatus: any, newStatus: any, createdBy: any) {
  ...
  async logAssignment(leadId: any, employeeName: any, createdBy: any) {
    return this.logActivity(
      leadId,
      'assignment',
  ...
  async logNote(leadId: any, note: any, createdBy: any) {
    return this.logActivity(
      leadId,
      'note',
      note,
  ...
  async getActivityStats(leadId: any) {
    const pool = await getMysqlPool()
