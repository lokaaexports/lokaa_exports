...
export class LeadService {
  ...
  // Create new lead
  static async createLead(leadData: any, createdBy: any) {
    try {
      const pool = await getMysqlPool()
  ...
  // Update lead
  static async updateLead(leadId: any, updateData: any) {
    try {
      const pool = await getMysqlPool()
  ...
  static async deleteLead(leadId: any) {
    try {
      const pool = await getMysqlPool()
      await pool.query(
  ...
  static async assignLead(leadId: any, employeeId: any) {
    try {
      const pool = await getMysqlPool()
      await pool.query(
