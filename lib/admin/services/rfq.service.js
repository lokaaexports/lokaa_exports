...
export class RFQService {
  async getAllRFQs(filters = {}, limit = 50, offset = 0) {
  ...
  async createRFQ(rfqData, createdBy) {
    const pool = await getMysqlPool()
  ...
  async updateRFQ(rfqId, updateData) {
    const pool = await getMysqlPool()
  ...
  async getRFQsByCustomer(customerId, limit = 20, offset = 0) {
    const pool = await getMysqlPool()
    const [rfqs] = await pool.query(
  ...
  async getRFQsByLead(leadId, limit = 20, offset = 0) {
    const pool = await getMysqlPool()
    const [rfqs] = await pool.query(
