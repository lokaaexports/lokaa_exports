...
export class EmployeeService {
  ...
  static async getEmployeeById(employeeId: any) {
    return await prisma.user.findUnique({
      ...
      include: {
        role: { include: { permissions: { include: { permission: true } } } },
        assignedCustomers: { include: { customer: true } },
        assignedTasks: {
          include: { task: true },
          take: 10,
  ...
  // Create new employee
  static async createEmployee(data: any, createdByUserId: any) {
  ...
  // Update employee
  static async updateEmployee(employeeId: any, data: any, updatedByUserId: any) {
  ...
  static async assignCustomerToEmployee(employeeId: any, customerId: any) {
  ...
  // Assign country to employee
  static async assignCountryToEmployee(employeeId: any, countryId: any) {
    return await prisma.countryAssignment.create({
      data: {
        userId: employeeId,
        countryId,
        assignedAt: new Date()
  ...
  static async getEmployeeAssignments(employeeId: any) {
    const [customers, countries, rfqs, tasks] = await Promise.all([
    ...
    return { customers, countries, rfqs, tasks }
  }
  ...
  static async deleteEmployee(employeeId: any) {
    ...
    await Promise.all([
      ...
      prisma.countryAssignment.deleteMany({ where: { userId: employeeId } }),
