...
export const authenticateToken = async (request: any) => {
  try {
    let token = null
...
export const requireRole = (requiredRoles: any) => {
  return (handler) => {
    return async (request, context) => {
      const auth = await authenticateToken(request)
...
export const extractAuthFromRequest = (request: any) => {
  const authHeader = request.headers.get('authorization')
  const token = authHeader && authHeader.split(' ')[1]
  return token
}

export const createAuthHeaders = (token: any) => {
  return {
