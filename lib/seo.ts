const SITE_URL = process.env.NEXT_PUBLIC_BASE_URL || 'https://lokaaexports.com'
...
export function buildMetadata({ title, description, path = '/', image = '/og-image.jpg', keywords = [], type = 'website' }: any) {
...
export function buildProductKeywords(product: any, categoryName = '') {
  const baseKeywords = [
    product.name,
    `${product.name} export`,
...
export function buildBreadcrumbSchema(items: any) {
  return {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
...
export function buildFaqSchema(items: any) {
  return {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
