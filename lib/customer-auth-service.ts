...
async function throttle(scope: any, identifier: any, limit: any, windowMs: any) {
  const now = new Date()
...
async function storeToken({ email, customerId, type, token, expiresAt }: any) {
...
async function consumeToken({ email, type, token }: any) {
...
export async function verifyEmailOTP(email: any, otp: any) {
  const normalizedEmail = String(email).trim().toLowerCase()
...
export async function loginCustomer(email: any, password: any) {
  const normalizedEmail = String(email).trim().toLowerCase()
  const customer = await prisma.customer.findUnique({ where: { email: normalizedEmail } })
...
export async function initiateForgotPassword(email: any) {
  const normalizedEmail = String(email).trim().toLowerCase()
  const customer = await prisma.customer.findUnique({ where: { email: normalizedEmail } })
...
export async function resetPasswordWithOTP(email: any, otp: any, newPassword: any) {
  const normalizedEmail = String(email).trim().toLowerCase()
...
export async function getCustomerProfile(customerId: any) {
...
export async function updateCustomerProfile(customerId: any, data: any) {
...
export async function getCustomerRFQs(customerId: any) {
