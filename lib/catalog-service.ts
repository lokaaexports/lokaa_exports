...
// New product management should use Prisma DynamicProduct-backed services under /api/admin/catalog.
...
function ensureUniqueSlug(value: any, fallbackPrefix: any, usedSlugs: any) {
...
function normalizeEntityList(items: any, fallbackPrefix: any, usedSlugs: any, fallbackName: any) {
...
function normalizeCatalogForSave(catalog: any) {
...
async function writeCatalogFile(catalog: any) {
  const normalized = {
...
function mergeCatalogData(fileCatalog: any, dbCatalog: any) {
  ...
  const categoryBySlug = new Map<string, any>(dbCategories.map((item: any) => [item.slug || item.name, item]))
  const mergedCategories = fileCategories.map((category: any) => ({
    ...category,
    ...((categoryBySlug.get(category.slug) || categoryBySlug.get(category.name) || {}) as object),
  }))
  ...
  const productBySlug = new Map<string, any>(dbProducts.map((item: any) => [item.slug || item.name, item]))
  const mergedProducts = fileProducts.map((product: any) => ({
    ...product,
    ...((productBySlug.get(product.slug) || productBySlug.get(product.name) || {}) as object),
  }))
...
async function writeSubcategories(subcategories: any, categories = []) {
...
export async function getProductsFromDb() {
  ...
  for (const row of rows) {
    const product: Record<string, any> = normalizeProductRow(row)
...
export async function getProductBySlugFromDb(slug: any) {
  const pool = await getMysqlPool()
  const [rows] = await pool.query('SELECT * FROM products WHERE slug = ? LIMIT 1', [slug])
  if (!rows.length) return null
  const product: Record<string, any> = normalizeProductRow(rows[0])
  const [attributeRows] = await pool.query(
    'SELECT a.slug, pav.value FROM product_attribute_values pav JOIN attributes a ON a.id = pav.attributeId WHERE pav.productId = ? ORDER BY a.sortOrder ASC',
    [rows[0].id]
  )
...
async function syncPimEntities(catalog: any) {
...
export async function saveCatalogData(catalog: any) {
...
export async function updateProductBySlug(slug: any, product: any) {
  const pool = await getMysqlPool()
  const now = new Date().toISOString().slice(0, 19).replace('T', ' ')
  const gallery = JSON.stringify(product.gallery || [])
  const certifications = JSON.stringify(product.certifications || [])
...
export async function deleteProductBySlug(slug: any) {
  const pool = await getMysqlPool()
...
export async function updateCategoryBySlug(slug: any, category: any) {
  const pool = await getMysqlPool()
  const now = new Date().toISOString().slice(0, 19).replace('T', ' ')
  const keywords = JSON.stringify(category.keywords || [])
  await pool.query(
...
export async function deleteCategoryBySlug(slug: any) {
  const pool = await getMysqlPool()
