...
function normalizeSimpleEntityRow(row) {
  return {
    id: row.id,
    slug: row.slug,
    name: row.name,
...
export async function createPackagingType(payload) {
  const pool = await getMysqlPool()
  const now = toSqlDate()
  await pool.query(
...
export async function updatePackagingType(id, payload) {
  const pool = await getMysqlPool()
  const now = toSqlDate()
  await pool.query(
...
export async function createExportCountry(payload) {
  const pool = await getMysqlPool()
  const now = toSqlDate()
  await pool.query(
...
export async function updateExportCountry(id, payload) {
  const pool = await getMysqlPool()
  const now = toSqlDate()
  await pool.query(
...
export async function deleteExportCountry(id) {
  const pool = await getMysqlPool()
