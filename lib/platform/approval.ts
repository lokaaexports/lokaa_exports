...
function normalizeApproval(row: any) {
  return {
    id: row.id,
...
export async function listApprovals({ status, domainName, limit = 100, offset = 0 }: { status?: string; domainName?: string; limit?: number; offset?: number } = {}) {
...
export async function updateApprovalStatus(id: any, status: any, notes = '') {
  const pool = await ensureTable()
  const now = new Date().toISOString().slice(0, 19).replace('T', ' ')
...
export async function getApprovalById(id: any) {
  const pool = await ensureTable()
