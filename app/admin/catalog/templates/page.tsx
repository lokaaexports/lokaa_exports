...
import { Category, Subcategory, Template, TemplateField } from '@/types/admin'
...
export default function TemplatesPage() {
  ...
  const [fieldForm, setFieldForm] = useState<Partial<TemplateField>>({ fieldName: '', fieldLabel: '', fieldType: 'TEXT', isRequired: false, displayOrder: 0, placeholder: '', helpText: '', options: '' })
  ...
  const loadCategories = async () => {
    const response = await fetch('/api/admin/catalog/categories', { headers: authHeaders() })
  ...
  const loadSubcategories = async () => {
    const response = await fetch('/api/admin/catalog/subcategories', { headers: authHeaders() })
    const payload = await response.json()
  ...
  const loadTemplates = async (categoryId, subcategoryId) => {
    ...
    const response = await fetch(`/api/admin/catalog/templates?${params.toString()}`, { headers: authHeaders() })
    const payload = await response.json()
  ...
  const saveTemplate = async () => {
    try {
      ...
      const endpoint = templateForm.id ? `/api/admin/catalog/templates?id=${templateForm.id}` : '/api/admin/catalog/templates'
  ...
  const performDelete = async () => {
    if (!deleteId) return
    try {
      const response = await fetch(`/api/admin/catalog/templates?id=${deleteId}`, { method: 'DELETE', headers: authHeaders() })
      const payload = await response.json()
  ...
  const addField = () => {
    ...
    setTemplateForm((current) => ({
      ...current,
      fields: [...(current.fields || []), { ...fieldForm, id: crypto.randomUUID?.() || String(Date.now()) } as TemplateField],
    }))
    setFieldForm({ fieldName: '', fieldLabel: '', fieldType: 'TEXT', isRequired: false, displayOrder: 0, placeholder: '', helpText: '', options: '' })
  }
  ...
  return (
    <>
      <SectionPage
        ...
        links={[{ href: '/admin/catalog', label: 'Products' }, { href: '/admin/catalog/categories', label: 'Categories' }]}
        stats={[
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  ...
                  <select value={fieldForm.fieldType} onChange={(event) => setFieldForm({ ...fieldForm, fieldType: event.target.value as TemplateField['fieldType'] })} className="rounded-xl border border-slate-200 bg-transparent px-3 py-2 text-sm outline-none dark:border-slate-700">
                  ...
                  {(fieldForm.fieldType === 'DROPDOWN' || fieldForm.fieldType === 'MULTI_SELECT') && (
                    <textarea value={fieldForm.options} onChange={(event) => setFieldForm({ ...fieldForm, options: event.target.value })} placeholder="Options (comma separated)" rows={2} className="rounded-xl border border-slate-200 bg-transparent px-3 py-2 text-sm outline-none dark:border-slate-700 md:col-span-2" />
                  )}
                  ...
                  {(templateForm.fields || []).map((field) => (
                    <div key={field.id} className="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2 dark:border-slate-700">
                      <div>
                        ...
                        {(field.fieldType === 'DROPDOWN' || field.fieldType === 'MULTI_SELECT') && field.options && (
                          <p className="text-xs text-slate-400">Options: {field.options}</p>
                        )}
                      </div>
              ...
              <article key={template.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setDeleteId(template.id)} className="rounded-xl border border-red-200 px-3 py-2 text-xs font-semibold text-red-700 dark:border-red-900">Delete</button>
                  </div>
                </div>
