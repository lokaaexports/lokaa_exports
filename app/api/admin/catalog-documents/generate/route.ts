...
// POST /api/admin/catalog-documents/generate
...
export async function POST(request: any) {
  try {
    ...
    if (!auth.authenticated) {
      return Response.json({ success: false, error: 'Unauthorized', message: auth.error }, { status: 401 });
    }
    ...
    switch (type) {
      ...
      case 'selected':
        if (!products.length) {
          return Response.json(
            { success: false, error: 'Products selection required' },
            { status: 400 }
          );
        }
        catalogueBlob = await service.generateCompleteCatalogue({ products });
      ...
      default:
        return Response.json(
          { success: false, error: 'Invalid catalogue type' },
          { status: 400 }
        );
    }
    ...
    return Response.json(
      { success: false, error: 'Failed to generate catalogue', details: error.message },
      { status: 500 }
    );
  }
}
