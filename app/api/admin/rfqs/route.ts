...
import { safeParseInt, sanitizeSearch, whitelistStatus, rfqStatusWhitelist, rfqPriorityWhitelist } from '@/lib/sanitize'
...
export async function GET(request: any) {
  try {
    ...
    const status = whitelistStatus(searchParams.get('status'), rfqStatusWhitelist)
    const priority = whitelistStatus(searchParams.get('priority'), rfqPriorityWhitelist)
...
export async function POST(request: any) {
  try {
    ...
    logAdminRequest('POST', '/api/admin/rfqs', auth.user.id, { action: 'create' })
