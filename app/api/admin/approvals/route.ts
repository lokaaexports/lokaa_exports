...
export async function GET(request: any) {
  try {
    ...
    return NextResponse.json({ success: false, error: error.message || 'Unable to load approvals' }, { status: 500 })
  }
}
...
export async function POST(request: any) {
  try {
    ...
    return NextResponse.json({ success: false, error: error.message || 'Unable to create approval' }, { status: 500 })
  }
}
...
export async function PATCH(request: any) {
  try {
    ...
    return NextResponse.json({ success: false, error: error.message || 'Unable to update approval' }, { status: 500 })
  }
}
