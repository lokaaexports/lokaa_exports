...
function VerifyEmailContent() {
  ...
  // Previously this was hardcoded as '' — it must come from the URL ?email= param
        ...
        <CardContent className="pt-6">
          ...
          {success && (
            <Alert className="mb-4 border-green-200 bg-green-50">
              ...
              <AlertDescription className="text-green-800">Email verified. Redirecting to login.</AlertDescription>
            </Alert>
          )}
          {resendMessage && (
          ...
          <div className="mt-4 space-y-2 text-center text-sm">
            ...
            <Button variant="outline" className="w-full" onClick={handleResendOTP} disabled={resending}>
              {resending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Sending...</> : 'Resend OTP'}
            </Button>
          </div>
          <div className="mt-4 text-center text-sm">
            <Link href="/auth/register" className="text-navy hover:text-gold">Back to registration</Link>
          </div>
        </CardContent>
      </Card>
    </AuthShell>
