...
function Field({ label, value, onChange, placeholder, type = 'text', inputMode, pattern, title, maxLength }: any) {
  return (
    <div>
...
function SearchSelect({ value, onChange, items, placeholder = 'Type to search…', getLabel, getKey }: any) {
...
function CountrySelect({ value, onChange, placeholder = 'Type to search…' }: any) {
...
function PortSelect({ value, onChange, placeholder = 'Type to search…' }: any) {
...
function PhoneCodeSelect({ value, onChange }: any) {
  const [open, setOpen] = useState(false)
  const [query, setQuery] = useState('')
  
  const currentCountry = SORTED_COUNTRIES.find(c => c.dial_code === value) || SORTED_COUNTRIES.find(c => c.code === 'IN')
  ...
  return (
    <div className="relative w-[130px] flex-shrink-0">
      <button
        ...
        className="flex w-full items-center justify-between rounded-xl border border-navy/15 bg-ivory px-4 py-3.5 text-navy text-sm transition focus:border-gold focus:outline-none focus:ring-4 focus:ring-gold/15"
      >
        <span className="flex items-center gap-2 truncate">
          <span className="text-base">{currentCountry ? getFlagEmoji(currentCountry.code) : ''}</span> 
      ...
      <AnimatePresence>
        {open && (
          <motion.div 
            ...
            className="absolute z-50 mt-2 w-[280px] rounded-xl border border-navy/10 bg-white shadow-xl overflow-hidden"
          >
            <div className="p-2 border-b border-navy/5">
              <input
                autoFocus
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Search country or code..."
                className="w-full bg-slate-50 text-sm px-3 py-2 rounded-lg outline-none focus:ring-2 ring-gold/20"
              />
            </div>
            <ul className="max-h-64 overflow-auto py-1">
              {filtered.length === 0 ? (
                <li className="p-4 text-sm text-graphite/60 text-center">No results</li>
              ) : (
                filtered.map((item) => (
                  <li key={item.code}>
                    <button
                      type="button"
                      onMouseDown={(e) => e.preventDefault()}
                      onClick={() => { onChange(item.dial_code); setOpen(false); setQuery('') }}
                      className="flex w-full items-center gap-3 px-4 py-2.5 text-left text-sm hover:bg-navy/5 transition-colors"
                    >
                      <span className="text-base">{getFlagEmoji(item.code)}</span>
                      <span className="truncate flex-1 font-medium">{item.name}</span>
                      <span className="text-graphite/50 text-[13px]">{item.dial_code}</span>
                    </button>
                  </li>
                ))
              )}
...
function RFQInner({ products = [], categories = [] }: any) {
  ...
  return (
    <>
      <Nav theme="light" categories={categories} />
                      ...
                      <div className="grid gap-5 md:grid-cols-2">
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            <PhoneCodeSelect value={form.phoneCode} onChange={(v: any) => update('phoneCode', v)} />
                            <input
                              type="tel"
                              inputMode="tel"
                              pattern="^[+\d\s()-]{7,64}$"
                              ...
                              className="flex-1 min-w-0 rounded-xl border border-navy/15 bg-ivory px-4 py-3.5 text-navy placeholder:text-graphite/40 transition focus:border-gold focus:outline-none focus:ring-4 focus:ring-gold/15"
                            />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid gap-5 md:grid-cols-2">
                        <div className="md:col-span-2">
                          ...
                          <select value={form.productInterest} onChange={(e) => update('productInterest', e.target.value)} className="w-full rounded-xl border border-navy/15 bg-ivory px-4 py-3.5 text-navy transition focus:border-gold focus:outline-none focus:ring-4 focus:ring-gold/15">
                            ...
                            {(products || []).map((p: any) => <option key={p.slug || p.id} value={p.name}>{p.name}</option>)}
...
export default function RFQPageClient({ products = [], categories = [] }: any) {
  return (
    <main>
      <RFQInner products={products} categories={categories} />
    </main>
  )
}
