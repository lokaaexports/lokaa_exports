...
function Counter({ to, suffix = '', duration = 2 }: any) {
  ...
  useEffect(() => {
    if (!inView) return
    const controls = animate(0, to, { duration, ease: [0.16, 1, 0.3, 1], onUpdate: (v) => setValue(v) })
...
function Reveal({ children, delay = 0, y = 28 }: any) {
  return (
    <motion.div
      initial={{ opacity: 0, y }}
...
export default function HomePageClient({ categories = [] }: any) {
  return (
    <main className="relative">
      <Nav theme="solid" categories={categories} />
      <Hero />
      <BrandsSection />
