    ...
    <footer className="relative bg-navy-deep text-white pt-24 pb-10 overflow-hidden">
      ...
      <div className="relative max-w-[1400px] mx-auto px-6 lg:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-14 pb-16">
          ...
          <div className="lg:col-span-2">
            <div className="text-[11px] tracking-[0.24em] text-gold uppercase mb-5">Categories</div>
            <ul className="space-y-3 text-sm text-white/70">
              {categories.slice(0, 5).map(cat => (
                <li key={cat.slug}><Link href={`/category/${cat.slug}`} className="hover:text-gold">{cat.name}</Link></li>
              ))}
              {(!categories || categories.length === 0) && (
                <li><Link href="/products" className="hover:text-gold">View Catalog</Link></li>
              )}
            </ul>
          </div>
