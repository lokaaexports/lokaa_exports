...
export default function ImageUploader({ 
  ...
  const fetchImages = async () => {
    try {
      ...
      const response = await fetch(
        `/api/admin/catalog/images?productId=${productId}`
      )
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      // 1. Upload file to local /api/uploads service
      const formData = new FormData()
      formData.append('image', file)
      formData.append('type', 'product')
      formData.append('entityType', 'product_image')
      ...
      // 2. Save metadata to product images table
  ...
  const handleDeleteImage = async (imageId) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/catalog/images?id=${imageId}`,
        { method: 'DELETE' }
      )
      const data = await response.json()
              ...
              {imagesByType[type].length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-4">
                  {imagesByType[type].map((image) => (
                    <div key={image.id} className="relative border rounded-lg overflow-hidden group h-32">
                      <Image
                        src={image.imageUrl}
                      ...
                      <div className="text-xs text-gray-600 p-2 bg-gray-50 truncate">
                        {image.imageTitle}
                      </div>
                    </div>
                  ))}
